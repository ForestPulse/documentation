...
For the ForestPulse project, Work Package 4.2 focuses on establishing the methodological basis for area-based derivation of forest structure attributes using the preprocessed ALS data from Work Package 4.1. The ALS data were harmonized, reprojected to EPSG:3035, and retiled into 1 km × 1 km subtiles. Each subtile fits perfectly within a FORCE cube tile (30 km × 30 km), requiring exactly 900 subtiles to cover each FORCE cube domain.
...
| Metric | German | Unit |
|---|---|---|
| Canopy cover | Überschirmungsgrad | (fraction of 1) |
...
covered by tree crowns, measured as the vertical projection onto a horizontal plane.

To compute CC from the LiDAR data, a Canopy Height Model (CHM) is created first.
The CHM has a resolution of 0.5 m and captures the highest lidar return in each
raster cell, whereby a lidar footprint with radius 25 cm is assumed for every 
point in the point cloud. 

Canopy Cover is calculated as the fraction of the CHM that is higher than 2 m.

This means that an unforested raster cell has a CC value of 0, whereas a CC 
value of 1 means that the area is fully covered by tree crowns.

---

### 2. Top Height (Bestandesoberhöhe)

In operational forestry, top height is defined as the mean height of the 
tallest trees, typically the 100 largest-diameter trees per hectare.

The **95th-percentile height (p95)** is a robust, reproducible estimator of 
upper canopy height from LiDAR data. Using a high percentile rather than the
absolute maximum captures the general top-of-canopy level while filtering out
outlier points (e.g. bird strikes or noise spikes).

LiDAR-derived p95 correlates strongly with stand height, timber volume, and 
above-ground biomass. Research has shown that LiDAR-based top height can predict 
field-measured dominant height with only a few percent error.

---

### 3. Vertical Structure / Layering (Bestandesschichtung)

The **Vertical Complexity Index (VCI)** quantifies the evenness of LiDAR point 
distribution across vertical canopy layers. It is computed as the normalized 
Shannon–Wiener entropy of the height distribution:

$$VCI = \frac{-\sum_{i=1}^{n} p_i \ln(p_i)}{\ln(n)}$$

Where $p_i$ is the proportion of returns in height bin $i$, and $n$ is the total 
number of height bins. In this case, 60 height bins of size 1 m were used. 
...
VCI captures the 3D structural diversity of the forest. High VCI indicates 
multi-layered canopies with understory, midstory, and overstory all present. It is 
commonly used alongside metrics such as FHD, VDR, and canopy cover to characterize 
...
Growing stock volume (GSV) is defined as the total above-ground stem volume of living 
trees per unit land area (m³/ha). 

At tree level, stem volume is estimated from DBH and height using allometric taper equations. 

**LiDAR-based estimation:** GSV is estimated using the Area-Based Approach (ABA).  
ALS-derived height metrics, as well as a wood density value which is derived from the species 
composition, serve as predictors in a parametric regression:
...
Using data of the German National Forest Inventory (BWI, Bundeswaldinventur), the model 
coefficients were calibrated to these values:
...
Above-Ground Biomass was calculated similarly to GSV, fitting a power law model from 
LiDAR metrics to BWI data. This formula:
...
### 6. Basal Area (Grundfläche)

Basal area (BA) is defined as the cross-sectional area of a tree stem at breast height 
(1.3 m above ground). At stand level, it is expressed as the sum of all stem cross-sectional
areas per unit land area (m²/ha). It is a widely used measure of stand density and stocking, 
closely related to timber volume and productivity.
...
**LiDAR-based estimation:** BA is estimated using the Area-Based Approach (ABA). A parametric 
regression model links ALS-derived height and density metrics to field-measured BA from BWI 
reference plots:
...
#### Point-based Canopy Cover (Überschirmungsgrad)
...
The variable "maxH" shows the height of the highest point whose lidar-footprint lies in the given pixel. 
Since outliers were removed in the preprocessing, it can be assumed to show the height of the highest 
vegetaion (or building).
 
---

#### Mean Top of Canpoy Heigt (Mittlere Höhe)

The variable "meanH" shows the mean height of a canopy height model (CHM).
The CHM with 0.5 m resolution was calulated from a las point cloud with R and lidR as follows:
...
While the variable itself is not of major interest for forestry practicioners, it serves as an important 
input for modeling other variables (see 4., 5. and 6.). 

---

#### Raster-based Canopy Cover (Überschirmungsgrad)

(of these, only "over02" is used under the name "CanopyCover", while the others are unused.)
